...
def main(dataframe : pd.DataFrame = None):
    ...
    if dataframe is None:
        ...
        if upload_file is not None:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=upload_file.name.split(".")[-1]) as temp_file:
                temp_file.write(upload_file.read())  # Salvar o conteúdo no arquivo temporário
                temp_file_path = temp_file.name  # Obter o caminho do arquivo temporário

            # Verificar a extensão do arquivo
            if temp_file_path.endswith(".pdf"):
                dataframe = pdf_to_dataframe(temp_file_path)
            elif temp_file_path.endswith(".csv"):
                dataframe = read_csv(temp_file_path)
            else:
                st.write("Tipo de arquivo não identificado.")
    ...
    if dataframe is not None:
        ...
        if selected_influencer:
            plot_influencer(selected_influencer, df)
